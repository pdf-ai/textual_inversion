...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        default=40,
        nargs="?",
        help="num of steps with in one sampling for the attacks",
    )
    ...
    parser.add_argument(
        ...
        default=0.1,
        nargs="?",
        help="epsilon for one step in one sampling",
    )
    ...
    try:
        ...
        for i, datas in enumerate(data.train_dataloader()):
            ...
            for j in range(data_source.shape[0]):
                ...
                save_adv = 'source/' + str(opt.output_path) + '/source_' + str(j+count) + '.png'
            ...
            print('target_dir', 'source/' + 'attacked_' + str(opt.output_path) + '/' + str(opt.epsilon) + '_' + str(opt.steps) + '_' + str(opt.alpha) +'_'  + str(opt.sampling_steps) + '_' + str(opt.sampling_alpha) +'/')
            
            epsilon = opt.epsilon/255.0 * (1-(-1))
            alpha = opt.alpha/255.0 * (1-(-1))
            steps = opt.steps
            sampling_steps = opt.sampling_steps
            sampling_alpha = opt.sampling_alpha/255.0 * (1-(-1))
            attack = LinfPGDAttack(net, fn, alpha, sampling_steps, sampling_alpha, clip_min=-1.0)
            input_data = data_source.cuda()
            current_input = input_data

            for t in tqdm(range(steps)):
                attack_img = attack.perturb(current_input, torch.zeros(data_source.shape))
                delta = attack_img - input_data
                delta = batch_clamp(epsilon, delta)
                delta = clamp(input_data + delta, -1.0, 1.0) - input_data
                current_input = input_data + delta

            if opt.visualize_debug:
                import matplotlib.pyplot as plt
                axis_x = np.arange(len(global_loss))
                plt.plot(axis_x,global_loss)
            ...
            for j in range(data_source.shape[0]):
                ...
                save_adv = torch.clamp((img  + 1.0) / 2.0, min=0.0, max=1.0).detach()
                grid_adv = 255. * rearrange(save_adv, 'c h w -> h w c').cpu().numpy()
                save_adv = 'source/' + 'attacked_' + str(opt.output_path) + '/' + str(opt.epsilon) + '_' + str(opt.steps) + '_' + str(opt.alpha) +'_'  + str(opt.sampling_steps) + '_' + str(opt.sampling_alpha) +'/' + str(j+count) + '.png'
                Image.fromarray(grid_adv.astype(np.uint8)).save(save_adv)
