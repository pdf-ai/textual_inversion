...
def patch_attack(image, applied_patch, mask, target, probability_threshold, net, lr=1, max_iteration=100):
    net.eval()
    applied_patch = torch.from_numpy(applied_patch)
    applied_patch = (applied_patch - 0.5) * 2
    mask = torch.from_numpy(mask)
    target_probability, count = 0, 0
    perturbated_image = torch.mul(mask.type(torch.FloatTensor), applied_patch.type(torch.FloatTensor)) + torch.mul((1 - mask.type(torch.FloatTensor)), image.type(torch.FloatTensor))
#     per_image = torch.zeros([4, perturbated_image.shape[1], perturbated_image.shape[2], perturbated_image.shape[3]])

#     for k in range(4):
#         per_image[k] = image[0]
#     per_image = per_image.cuda()
#     ini_loss = net(per_image)
#     print('ini_loss', ini_loss)
    for count in tqdm(range(max_iteration)):
        ...
        for k in range(4):
            per_image[k] = perturbated_image[0]
        per_image = per_image.cuda()
        ...
        print(output)
        print(torch.max(per_image), torch.min(per_image))
        print(torch.max(applied_patch), torch.min(applied_patch))
        print(torch.max(lr * patch_grad))
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(opt.embedding_manager_ckpt)
        ...
        for i, datas in enumerate(data.train_dataloader()):
            data_source = model.get_source_input(datas, 'image')
            input_size = 256
            target_dir = os.path.join(os.getcwd(), opt.data_root)
            print(target_dir)
            input_image_dir = os.listdir(target_dir)
            for j in range(len(input_image_dir)):
                # print(opt.data_root)
                image_path = os.path.join(target_dir, input_image_dir[j])
                ...
                data_source[j] = trans(img)

            data_ = model.get_input(datas, 'image')
            net = target_model(model, data_[1])
            net = net.cuda()
            epsilon = opt.epsilon/100.0
            alpha = opt.alpha/255.0 * (1-(-1))
            steps = opt.steps
            input_data = data_source.cuda()
            print(net(input_data))
            attack_img = torch.zeros(input_data.shape).cuda()
            for j in range(len(input_image_dir)):
                patch = patch_initialization('rectangle', image_size=(3, 256, 256), noise_percentage=epsilon)
                image = input_data[j].unsqueeze(0)
                applied_patch, mask, x_location, y_location = mask_generation('rectangle', patch, image_size=(3, 256, 256))
                perturbated_image, applied_patch = patch_attack(image, applied_patch, mask, None, 0, net, 1.0, opt.steps)
                perturbated_image = torch.from_numpy(perturbated_image).cuda()
                attack_img[j] = perturbated_image[0]

            data_source_ = attack_img

            for j in range(data_source.shape[0]):
                img = data_source_[j]
                ...
                save_adv = 'source/' + 'attacked_' + str(opt.output_path) + '/' + str(opt.steps) + '_patch_' + str(epsilon) + '/' + str(j+count) + '.png'
                Image.fromarray(grid_adv.astype(np.uint8)).save(save_adv)
            count += data_source.shape[0]
            exit()
    except Exception:
