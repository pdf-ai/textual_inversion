...
class DDPM(pl.LightningModule):
    ...
    def p_losses(self, x_start, t, noise=None):
        if self.x_start == None:
            self.x_start = torch.randn_like(x_start)
        noise = default(noise, lambda: self.x_start)
...
class LatentDiffusion(DDPM):
    ...
    def get_source_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
    ...
    def forward(self, x, c, *args, **kwargs):
        if self.count_t == 0:
            ...
            self.t = t
        self.count_t += 1
        if self.count_t == self.resampling_t:
            self.count_t = 0
        if self.model.conditioning_key is not None:
        ...
        return self.p_losses(x, c, self.t, *args, **kwargs)
