...
def patch_initialization(patch_type='rectangle', image_size=(3, 256, 256), noise_percentage=0.03):
...
# TODO: Add circle type
def mask_generation(mask_type='rectangle', patch=None, image_size=(3, 256, 256)):
...
def test_patch(patch_type, target, patch, test_loader, model):
    ...
    for (image, label) in test_loader:
        ...
        if predicted[0] != label and predicted[0].data.cpu().numpy() != target:
            test_actual_total += 1
            applied_patch, mask, x_location, y_location = mask_generation(patch_type, patch, image_size=(3, 256, 256))
